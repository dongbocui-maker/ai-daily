...
<p class="text-[15px] text-accent-gray-800 leading-relaxed">负责 PDF 支持、citations、web search 等
**Built complex applications in unfamiliar languages**
Created 5,000-line TypeScript applications despite having minimal JavaScript/TypeScript experience.
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 6. API（API Knowledge 团队）</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">开发 PDF 支持、citations、web search 等"把外部知识引入 Claude 上下文"的功能。痛点是经常碰陌生代码段、建立上下文很费时。他们把 Claude Code 当成"any task 的 first stop"——先让它定位相关文件再开工；不熟的代码段的 bug 直接问"能修吗？"；通过 dogfooding 体验最新 model snapshot；省掉"拖文件到 Claude.ai"的 overhead。关键收益：陌生领域信心提升；轮岗工程师更快上手；developer happiness 显著上升。方法论：把它当迭代 partner，而非一击命中的 "solver"；从最小信息起步，让 Claude 自己挖。</p>
</aside>
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 7. Growth Marketing（增长营销）</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">"非技术单兵团队"——但产出像一支完整工程团队。痛点：海量重复创意、跨平台数据要查、A/B 测试要追踪。典型用法：搭 agentic workflow 批量生成 Google Ads（headline / description 两个特化子代理分管字符数限制）；做 Figma 插件批量替换文案出图（100 个变体半秒搞定）；做 Meta Ads MCP server 直接在 Claude Desktop 查营销数据；做 memory 系统记录假设和实验、自我迭代。关键收益：广告文案从 2 小时压到 15 分钟；创意产出 10 倍；一个人当一个团队用。金句：找带 API 的重复活就是金矿。</p>
</aside>
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 8. Product Design（产品设计）</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">支持 Claude Code / Claude.ai / Anthropic API 的产品设计。痛点：视觉细节调整（字号、配色、间距）反复 ping 工程师；mockup 翻译成代码要解释半天。典型用法：自己直接改 state management（连工程师都惊讶）；GitHub Action 接 issue 自动出 PR；把 mockup 截图喂给 Claude 直接生成可运行原型；设计阶段就 map 出错误态/逻辑流；改全站 copy（如"research preview"提示）实时协同法务迭代。关键收益：Figma 和 Claude Code 80% 时间都开着；执行加速 2-3 倍；一周协调缩成两个 30 分钟通话。关键诀窍：自定义 memory 文件告诉 Claude"我是设计师，小步走、多解释"。</p>
</aside>
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 9. RL Engineering（强化学习工程）</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">负责 RL 高效采样和跨集群权重传输。痛点：debug 复杂、call stack 深、K8s 操作命令多。典型用法：写中小特性时让 Claude 主导、自己监督；先自己改、再让 Claude 补测试或 review；用 Claude 快速总结调用栈替代手工读代码；K8s 命令直接问。关键收益：一次成功率约 1/3，但即使不一次成功，剩下 2/3 切到协作模式仍能省时间。核心诀窍：先 one-shot 试运气；CLAUDE.md 写极细规则（如"跑 pytest 别 cd"）防工具调用错；commit-heavy 工作流随时回滚。诚实评价：comment 有时加在奇怪位置，代码组织偶尔差强人意。</p>
</aside>
...
They now use a "try and rollback" methodology, frequently committing checkpoints so they can test Claude's autonomous implementation attempts and revert if needed, enabling more experimental development.
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 10. Legal（法务团队）</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">法务团队（产品律师为主），靠实验和好奇心发现 Claude Code，其中一名成员有"为家人做无障碍工具"的强烈个人动机。典型用法：1 小时建出帮有发音障碍家人的"预测文本+语音库" app；做内部"电话树"帮员工找对应律师；G Suite 应用自动化每周团队 update 和 legal review 状态；快速做出原型给 UCSF 等领域专家看验证想法。关键收益：法务也能不靠工程团队自建工具；prototype-driven 文化传播。核心方法：Plan in Claude.ai, build in Claude Code（先对话界面 brainstorm，再让 Claude Code 一步步实现）。独特视角：作为产品律师同时识别 MCP 集成的安全合规边界——AI 工具访问越深，合规建设要越快。</p>
</aside>
...
把额外知识带进 Claude context window 的功能。痛点是必须在大型复杂 codebase 上工作——总在遇到陌生模块、花大量时间收集上下文。Claude Code 成了他们的 "first stop"：先问它哪些文件相关，再开始干活。意外收益是 model dogfooding——Claude Code 自动用最新的研究模型快照，让他们提前感知模型行为变化。</p>
</aside>
