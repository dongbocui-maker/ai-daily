...
1. **Redesign work around agentic-first workflows**：不是把 AI 套到既有流程上，而是从头想象「如果这个流程一开始就有 AI agent，它会长什么样」。65% 的 agentic 领先者预期 operating model 改变。

1. **Upgrade governance and redesign decision rights**：58% 领先者预期决策权和治理调整；创建 governance hubs + 企业级 guardrails；treat decision rights as dynamic（按 workflow 量身定制）。

1. **Redefine roles for human-agentic interaction**：45% 领先者预期 middle management 层减少；建立 dual career paths（generalist orchestrators + AI-augmented specialists）。

1. **Prioritize continuous learning for humans AND AI agents**：员工培训要包含“如何监督、批评、指导 AI”；AI Agent 要做“performance review”，必要时 retrain。

1. **Anchor investment decisions in value**：投资必须锚定 value thesis（efficiency / innovation / revenue），不能为现代化而现代化。

**IDC「Digital Labor」论断**进一步深化：Agent 不是新工具，是新劳动力。这一定位的革命性在于——**资本支出与工资支出之间的边界开始模糊**。MIT Sloan 把这归为四张力中的「Investment vs Employment」：「Traditional tools require large upfront costs but deliver predictable returns through established depreciation schedules. Human workers are an ongoing variable expense, but their value appreciates with experience and training. Agentic AI defies both models, requiring substantial initial development costs and ongoing variable costs.」
