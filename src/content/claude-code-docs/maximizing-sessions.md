---
slug: maximizing-claude-code-sessions
title: "Maximizing the Value of Your Claude Code Sessions"
subtitle: "Anthropic 官方发布 · 让每个 token 都花在刀刃上的 session 运行方法"
sourceUrl: "https://claude.com/blog/maximizing-the-value-of-your-claude-code-sessions"
sourceLabel: "claude.com/blog · Lydia Hallie · 2026-08-14"
updated: "2026-09-02"
---
...
<li><strong>Session 的成本模型只有三个变量</strong>：多少 token 进了 context × 停留多少轮 × 同时跑几个 context。每一轮请求都会重发整个对话历史（靠 prompt cache 打到 0.1x），所以「什么进了 context」和「session 拖多久」直接决定账单。</li>
<li><strong>输出 token ≈ 5x 输入价，因为 decode 是逐 token 串行跑 GPU</strong>。一段 200 token 的回复 = 模型串行跑 200 次；其中大量是 thinking token，由 <code>/effort</code> 控制。</li>
<li><strong>Prompt cache 是省钱核心，但很容易被打破</strong>。cache 从请求最前缀开始逐字匹配：中途切 <code>/model</code>、<code>/effort</code>、开 fast mode、跑 <code>/compact</code>，都会让整个对话全价重新 prefill。要换就在 session 开头或 <code>/clear</code> 之后换。</li>
<li><strong>cache 有效期：订阅版 1 小时、API key 只有 5 分钟</strong>（<code>ENABLE_PROMPT_CACHING_1H=1</code> 可延到 1 小时）。离开键盘前先 <code>/compact</code>——对话还在 cache 里时做摘要便宜得多。</li>
<li><strong>@-mention 文件优于口头报路径</strong>：文件直接附进第一个请求，省掉 Read 调用甚至一轮搜索；但同一文件只 @ 一次，重复 @ 会附第二份副本。</li>
<li><strong>命令输出和文件一样会在 context 里住到 session 结束</strong>。400 行 passing 测试输出会被后面每一轮重复携带——常用命令连静默 flag 一起写进 CLAUDE.md（如 <code>--reporter=dot</code>）。</li>
<li><strong><code>/rewind</code> 优于 <code>/compact</code></strong>（当只想丢掉最后几轮时）：rewind 只砍尾部、前缀 cache 全保留、零成本；compact 重写整个对话必有开销。</li>
<li><strong>Subagent 的本质是「让脏活发生在别人的 context 里」</strong>：适合产出大量一次性输出的工作（翻日志、批量调查）；反复用的噪音型任务给它配独立 subagent 定义 + 小模型（haiku/sonnet）。</li>
</ul>
</aside>

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · TL;DR 与「为什么任务有了单价」</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">开篇先给六条速查（/clear 切任务、开局定 model+effort、@-mention 文件、静默 flag、/context 审计启动负载、离开前 /compact），然后点出一个思维转变：传统编辑器是包月的，任务没有单价；agentic 工具时代同一个任务因用法不同可以差出数倍成本。高效不是少用 token，而是让 token 都花在你真正要的事情上。</p>
</aside>
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 一个 token 的价格由什么决定</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">三个因子：模型大小（所有成本乘以模型单价——难题用大模型、常规活用小模型）、输入 vs 输出（prefill 并行读入很快，decode 逐 token 串行生成，所以输出 ≈ 5x 输入价；thinking token 归 /effort 管）、是否命中缓存（cache 读 0.1x、写最高 2x 但只写一次）。最值钱的知识是「什么会打破 cache」：/model、/effort、fast mode 都是 cache key 的一部分，/compact 会重写对话，超时（订阅 1h / API 5min）会让整个对话重新全价 prefill。文中用一个「修失败测试」的例子拆出 5 个请求，展示每轮账单 = cache 读历史 + 全价读新增 + 输出价。</p>
</aside>
...
![Image 1](https://cdn.prod.website-files.com/68a44d4040f98a4adf2207b6/6a7f1947bc7cd69c4c891a0f_c69dbb11.png)
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 一个 session 会发出多少 token</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">核心事实：没有任何东西只发送一次——进入对话的每个文件、每段命令输出，都会在之后每一轮被重发（cache 打折但不免费，还占模型思考空间）。三个抓手：① 控制什么进 context——开局 /context 审计启动负载，CLAUDE.md 只留具体指令、工作流指令挪进 skills，不用的 MCP server 用 /mcp 关掉；提示词说得越具体，Claude 自己乱翻的文件越少。② 控制停留轮数——一个 40 轮的长 session 每轮都在重读前 39 轮，/clear 切任务、/compact 收阶段；/loop 会带全量对话反复开火，建议另开终端跑。③ Subagent 隔离——独立 context 干完只回答案，适合大输出的一次性工作。</p>
</aside>
...
![Image 2](https://cdn.prod.website-files.com/68a44d4040f98a4adf2207b6/6a7f1b213f60488b546224d4_cab63270.png)
...
![Image 3](https://cdn.prod.website-files.com/68a44d4040f98a4adf2207b6/6a7f1cdb7fb1ad2229b0afa5_92ab0ee2.png)
...
![Image 4](https://cdn.prod.website-files.com/68a44d4040f98a4adf2207b6/6a7f1cdb7fb1ad2229b0afaa_a653b369.png)
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 优先看哪里</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">收尾给出按成本排序的四个巡检点：长 session 不清（最贵）、中途切 model/effort 打破 cache、噪音命令输出堆积、启动负载过重。如果只做一件事：任务切换时 /clear。</p>
</aside>
...
![Image 5](https://cdn.prod.website-files.com/68a44d4040f98a4adf2207b6/6a7f1dd4531c50c7022d5171_df696a6b.png)
