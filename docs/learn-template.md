...
## 六、文章页内容规范（2026-09-05 Jason 拍板，同时适用精读 reads 板块）

1. **头部链接区置顶**：文章正文最上方放数据源链接按钮组——官方原文必放；配套资源（课程版、视频、GitHub repo 等）如有一并置顶。
2. **PDF 下载**：数据源若提供 PDF 可下载版，必须在头部链接区提供下载入口；**源不提供则不自制 PDF**。
3. **图片本地化**：原文中的信息性示意图必须抓取到 `public/learn/<slug>/`（或 `public/reads/<slug>/`）站内引用，不外链源站 CDN（防失效）；每张配中文图注。装饰性图标不抓。
4. **内容完整性**：代码块、配置样例、表格完整保留；对照块（Traditional vs AI-native 之类）保持结构化排版。

## 七、版本历史

- **v1.1**（2026-09-05）：新增文章页内容规范（头部链接区 / PDF 下载 / 图片本地化）
