...
subtitle: "Anthropic 官方 · 当代码不再是瓶颈，如何逐阶段重构软件开发生命周期"
sourceUrl: "https://claude.com/blog/the-ai-native-sdlc-playbook"
sourceLabel: "claude.com/blog · Louis Claxton · 2026-08-21"
updated: "2026-09-05"
---

<div class="not-prose my-6 flex flex-wrap gap-3">
<a href="https://claude.com/blog/the-ai-native-sdlc-playbook" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2.5 bg-accent-ink text-white text-sm font-semibold rounded hover:bg-accent-purple transition">📖 官方原文（claude.com/blog）</a>
<a href="https://academy.claude.com/courses/ai-native-sdlc-playbook" target="_blank" rel="noopener noreferrer" class="inline-flex items-center gap-2 px-4 py-2.5 border border-accent-gray-300 text-sm font-semibold rounded hover:border-accent-purple hover:text-accent-purple transition">🎓 配套课程版（Claude Academy）</a>
</div>
...
<li><strong>代码不再是瓶颈，流程才是</strong>。agent 把 build 压缩到小时级后，瓶颈外移到仍以人速运行的 Plan / Review / Deploy；逐行人工 review 和周会治理在 agent 产出量面前失效，必须重构而非加人。</li>
<li><strong>SDLC 从直线变循环，靠「提交的 artifact」串联</strong>：每阶段以一个 git 提交收尾（<code>intent.md</code> → <code>spec.md</code> → <code>plan.md</code> → diff+tests → PR+审查记录 → 事故记录），下一阶段读取它自动开始。commit 链就是审计轨迹。</li>
<li><strong>想法一次捕获</strong>：任何角色与 Claude 头脑风暴直接产出 <code>intent.md</code>（proto-spec），需求+设计压缩为一个 session 产出 <code>spec.md</code>，组织政策以 skills 形式在撰写时实时约束，而不是几周后的评审会上才被发现。</li>
<li><strong>Build 阶段的知识工程化三件套</strong>：CLAUDE.md 沉淀团队知识（同一错误犯两次就写入）、skills 让制度可执行（advisory）、hooks 做确定性护栏（必须无例外成立的政策才用 hook 兜底）。skill 让违规变少，hook 让违规几乎不可能。</li>
<li><strong>给 agent 自验证回路是测试阶段的核心</strong>：测试/构建/截图 diff，session 自查自纠后才给人看。修 bug 先提交失败测试，再要求 agent 不改测试让其通过。配置（CLAUDE.md/skills/hooks）也要在 CI 里跑 continuous evals 防回归。</li>
<li><strong>治理总纲：agent 可以做到 production gate 之前的一切，但不能跨过 gate</strong>。写代码的 agent 无权批准自己的代码；发布授权永远来自具名的人；不可协商的管控放 managed settings，工程师关不掉。</li>
<li><strong>维护阶段闭环</strong>：监控脚本按 σ 分级响应（1σ 记录 / 2σ 只读诊断 / 3σ 可开 PR 或触发预批准 runbook），异常写回 <code>intent.md</code> 重新进入循环——loop 自我供给，人的判断留在循环之上。</li>
<li><strong>对受监管企业的诚意细节</strong>：legacy 系统（Jira/ServiceNow）source-of-truth 三种配置、受监管企业 managed settings 完整实例逐行解读、文末平台团队 rollout 文档清单——可直接作为企业级实施 checklist。</li>
</ul>
</aside>

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 代码不再是瓶颈</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">开篇立论：agent 把写代码的速度提升了一个量级，但代码周边的流程没变。传统 SDLC 六阶段（规划→设计→构建→测试→部署→维护）的审批门、逐行 review、周会治理，都是为「写代码最贵最慢」的时代设计的。当 build 塌缩到小时级，三件事随之成立——瓶颈外移到左右两侧的人速环节、逐行人工 review 跟不上 agent 产出量、例外审批仍走周会导致治理成本激增。安全团队是最典型的例子：按人类产出规模配置的安全审查，面对 agent 放大的代码量，要么排队要么欠审，受监管企业两者都不能接受。</p>
</aside>
...
![Build 阶段塌缩后，瓶颈转移到左右两侧的人速环节](/learn/ai-native-sdlc-playbook/bottleneck-shift.png)

*图：agent 出现前，六个阶段都以人速运行；agent 出现后 build 塌缩到小时级，而 plan、review、release 仍保持原有时长——瓶颈整体外移。*
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 什么是 AI 原生 SDLC</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">AI-native SDLC = 旧的控制目标 + 新的执行方式：流程从直线变成循环，AI 嵌入每个节点，阶段间交接由 artifact 提交自动触发。贯穿全文的核心设计是「提交的 artifact」——每个阶段以一个 git 提交收尾（intent.md → spec.md → plan.md → diff+tests → PR+审查记录 → 事故记录），下一阶段读取它开始工作。commit 链就是审计轨迹：谁要求了什么、agent 产出了什么、谁批准了。人类始终对需要判断力的决策负责，只是注意力随 artifact 移动。</p>
</aside>
...
![传统 SDLC 直线流程 vs AI-native 循环流程](/learn/ai-native-sdlc-playbook/ai-native-loop.png)

*图：传统 SDLC 是一条直线，任何回退都意味着新的发布周期；AI-native SDLC 是小时级的循环，人在循环之上发起、指挥、治理。*
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Plays 总览与采纳顺序</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">14 个 play 按六个非线性阶段分组，每个 play 讲五件事：改变了什么、如何起步、实施步骤、治理考量、如何度量。阶段编排是模块化的——组织可按自身需要优先改造不同阶段。注意「列出顺序」和「采纳顺序」不同：依赖图中无入边的 play（CLAUDE.md、plan mode、intent.md 等）可以直接开始，其他 play 先做指向它的前置项。终态是一个循环：intent.md 被接受触发需求设计、spec.md 获批触发 plan mode、PR 合并触发流水线、生产控制带被击穿写出下一个 intent.md。</p>
</aside>
...
![各 play 的采纳顺序依赖图](/learn/ai-native-sdlc-playbook/plays-adoption-order.png)

*图：14 个 play 的采纳顺序图。无入边的 play（如 CLAUDE.md、plan mode、intent.md）可以随时开始；其余 play 需先完成指向它的前置项。*

The plays are listed with stage; the arrows give the order to adopt them in. The two are not the same. Start with any clay play — nothing points into it, so it needs nothing first. For any other play, the arrows pointing into it are the plays to adopt before it.**01**

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Stage 1 · Plan——想法一次捕获为 intent.md</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">想法不再等人写成需求文档。发起人（可以是任何非工程角色）与 Claude 头脑风暴，产出 proto-spec 保存为 intent.md：问题、期望结果、受影响的用户和系统、约束、开放问题——用发起人自己的语言，人可读、机器可执行、版本受控。传统流程中想法要穿过 backlog、user story、估点、细化会议，每次交接都在失真；AI 原生流程里发起人直接落 artifact，产品负责人 review 后提交。度量：从对话到 intent.md 提交的时间（周级→小时级）、intent 存活率（被接受进设计的比例）。</p>
</aside>
...
> **Traditional（传统做法）**：An idea passes through backlog entries, user stories, story points, and refinement meetings before anyone can act on it. Ownership transfers at each handoff, so what reaches engineering is several steps removed from what the originator meant.
>
> **AI-native（AI 原生做法）**：The originator brainstorms with Claude and writes the result down as `intent.md`, a proto-spec in the originator's own terms. The artifact contains what is wanted, why, and under which constraints. Repeat processes are encoded via skills.**Getting started · 起步条件**

- **Prerequisites（前置）**：None.
- **Infrastructure（基础设施）**：Claude access for people who are not engineers (claude.ai or [Cowork](https://claude.com/product/cowork)); an agreed `intent.md` template; a shared, version-controlled home for intent that the product owner watches. For a single product the simplest home is an `intent/` folder in the product repo. This setup keeps the artifact chain next to the code derived from it. A dedicated intent repo is only worth the overhead when intent spans many repositories, and in a monorepo it is a directory. The Stage 3: Build sidebar covers how this home relates to a Jira or requirements tool that already holds the record.
...
<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Stage 2 · Design——需求与设计压缩为一个 session</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">需求分析和设计从两个团队的两个阶段压缩为一次 prompted session：Claude 读取已接受的 intent.md，在组织的 brand/安全/合规/UX skills 约束下产出 spec.md，并主动标记冲突点（analyst 会上报的那些问题）。产品负责人只 review 不撰写，优先处理被标记的关注点并与政策 owner 逐一解决。政策在写 spec 时实时生效，而不是几周后的评审会上才被发现。前端场景最典型：intent.md 接受后用 Claude Design 出 mock，迭代后导出给 Claude Code 实现。</p>
</aside>
...
> **Traditional（传统做法）**：Requirements and design are separate phases run by separate teams. Analysts formalize the idea into requirements and designers then parse those back into a design. The separation exists for accountability, but it is slow and lossy.
>
> **AI-native（AI 原生做法）**：Both phases happen in a single prompted session. Claude takes `intent.md` and produces a requirements and design spec, constrained by the organization's skills, with areas of concern flagged.**Getting started · 起步条件**

- **Prerequisites（前置）**：Write an `intent.md` file, with brand, security, compliance, and UX policies written as skills.
- **Infrastructure（基础设施）**：A product owner with Claude access. No engineering skill is required.
...
- **Leading indicator（先行指标）**：Elapsed time between the `intent.md` commit and the `spec.md` commit for the same change (two git timestamps), compared with the old requirements-plus-design cycle.
- **Lagging indicator（滞后指标）**：Requirements rework after build starts. Count `spec.md` commits dated after the first `plan.md` commit for the same change. Git log will give this directly.

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Stage 3 · Build——本文最重的一章：plan mode、CLAUDE.md、skills、hooks、并行 session</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">五个 play 密集展开。① Plan mode 为默认起点：没有被接受的计划就不写代码，计划提交为 plan.md 进入审计链，后续 PR review 会对照它检查。② CLAUDE.md 是团队制度化知识的落点——「同一个错误犯两次就写进 CLAUDE.md」，保持一页以内。③ Skills 让制度知识可执行：必须一致执行的政策写成 skill，advisory 性质，必须无例外成立的政策要再配 hook。④ Hooks 是 skill 背后的确定性层：锁保护路径、跑 formatter、防凭证入 diff。⑤ 并行 session + subagent：一个工程师用 git worktree 同时开 2-3 个独立 session，重复性工作固化为 subagent 定义（verifier / simplifier / researcher），工程师的工作变成编排和 review。另附 sidebar 讲 legacy 系统（Jira/ServiceNow）与 markdown artifact 的 source-of-truth 三种配置——对已有合规体系的企业尤其重要。</p>
</aside>
...
> **Traditional（传统做法）**：An engineer reads the design and starts writing code. How the change will be made, down to which files and which tests, stays in the engineer's head or at best a ticket comment. Nobody else can review it. The first thing a reviewer sees is the finished diff, and by then rework is slow.
>
> **AI-native（AI 原生做法）**：Work starts with a written plan that Claude produces in plan mode, where it can read the codebase without changing anything. The engineer corrects the plan before code is written, and the approved version is committed as `plan.md` for later stages to check against.**Getting started · 起步条件**

- **Prerequisites（前置）**：The intent artifact (`intent.md` or `spec.md`) if one exists, and the `CLAUDE.md` file helps.
- **Infrastructure（基础设施）**：Claude Code with access to the repository.
...
- **Leading indicator（先行指标）**：Share of changes that merge from the first implementation pass, and time from plan approval to merged PR with the required data within the PR metadata.
- **Lagging indicator（滞后指标）**：Rework cycles per change, again from the PR metadata, and how often the merged diff still matches the committed `plan.md`.
...
**Getting started · 起步条件**

- **Prerequisites（前置）**：None.
- **Infrastructure（基础设施）**：A repo, Claude Code installed, and one engineer who knows the codebase well.
...
- **Leading indicator（先行指标）**：How often Claude repeats a mistake `CLAUDE.md` should have caught. The corrections or changes to the `CLAUDE.md` should be tracked within the git history.
- **Lagging indicator（滞后指标）**：Time to first merged PR for a new member of the team from PR history.
...
- **Prerequisites（前置）**：None required. Having a `CLAUDE.md` helps, because it keeps the agent's working knowledge in the repo, but a skill does not depend on it.
- **Infrastructure（基础设施）**：One policy with a named owner and a written source of truth.
...
- **Leading indicator（先行指标）**：Time from the policy owner approving a policy change to the updated skill merging, taken from the PR on the skill folder.
- **Lagging indicator（滞后指标）**：PR reviews findings that cite the policy, which should fall towards zero once the skill is applying the policy while the code is written. Where the findings don't fall towards zero, either the skill isn't triggering or its text has drifted from the official policy.
...
> **Traditional（传统做法）**：One engineer works one task at a time and spends a significant portion of their day or week on builds, tests and reviewers. Switching between tasks while waiting is possible, but the context switch is tiring enough that few people choose to.
>
> **AI-native（AI 原生做法）**：One engineer runs several Claude sessions at once, each in its own worktree on its own task. Repeated jobs become subagents with their own context and tool limits. The engineer's job shifts to orchestrating, and eventually, to building and monitoring loops.**Getting started · 起步条件**

- **Prerequisites（前置）**：The `CLAUDE.md`, since all sessions read the file. The feedback loop (Stage 4: Test) also helps here, because less supervision from the engineer is needed when a session can verify its own work.
- **Infrastructure（基础设施）**：A git repository, since isolation comes from worktrees and permission settings tuned so sessions are not waiting on approval prompts for commands the organization considers safe.
...
- **Leading indicator（先行指标）**：Concurrent sessions per engineer while review quality holds, counted from the OpenTelemetry export, and the share of the day spent steering rather than waiting.
- **Lagging indicator（滞后指标）**：Changes merged per engineer per week read alongside the rework rate as determined per the PR history.

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Stage 4 · Test——agent 自验证 + 配置也要回归测试</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">两个 play。① 反馈回路：永远给 Claude 一条自验证路径（测试/构建/截图 diff），session 在人看到之前自查自纠。修 bug 先写失败测试并提交，然后要求 Claude 在不改测试的前提下让它通过（hook 强制），这才是 bug 修复的证明。回路本身要保护——修代码的 agent 不能有能力弱化对代码的检查。② Continuous evals：CLAUDE.md、skills、hooks 这些「引导 agent 的配置」和代码一样会回归，要在 CI 里定期跑 eval 套件验证；每次生产事故固化为一条永久 eval。QA 的门从阶段边界移到持续运行。</p>
</aside>
...
> **Traditional（传统做法）**：The signal that code works arrives late. CI minutes later, a tester days later, production weeks later. With an agent producing the code, a late signal means a person has to check all of its output, and that person becomes the bottleneck.
>
...
- **Prerequisites（前置）**：None.
- **Infrastructure（基础设施）**：A test suite and a build that run locally with one command each. For the UI work, a way for Claude to see the result is crucial, either a browser tool or a screenshot utility wired in via MCP.
...
- **Prerequisites（前置）**：The `CLAUDE.md` and feedback loop (Stage 4: Test).
- **Infrastructure（基础设施）**：CI that can run Claude Code non-interactively, and an API key with budget for eval runs.
...
- **Leading indicator（先行指标）**：The eval pass rate over time, reported by the suite on every run, and how long a production incident takes to become a permanent eval.
- **Lagging indicator（滞后指标）**：Regressions caught in CI compared with regressions found in production derived from the incident tracker.

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Stage 5 · Deploy——双向 review、hooks 审批门与企业级管控</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">三个 play。① AI 进入 PR review 循环：Claude 既审别人的 PR（按 REVIEW.md 定义的 bugs/安全/合规三遍扫描，findings 分级），也处理自己 PR 收到的评论（@claude 自动修复推送）；人的注意力上移到「这个变更是否符合 plan 的意图、风险是否可接受」。写代码的 agent 无权批准自己的代码，审批仍来自 branch protection 里的人类 code owner。② Hooks 升级为审批门：构建期 hook 是自动放行/拦截，发布门 hook 是「暂停等指定人批准」；不可协商的 hook 放进 managed settings，工程师关不掉。附完整的受监管企业 managed settings 实例逐行解读（sandbox、凭证隔离、插件白名单、最低版本门槛）——这段对企业落地极具参考价值。③ CI/CD 集成：claude -p 非交互跑在流水线里做判断类工作（triage 失败构建、写 changelog），部署经 MCP 暴露为工具按环境分级授权，rollback 是全流水线最需要反复演练的路径。治理总纲：agent 可以做到 production gate 之前的一切，但不能跨过 gate。</p>
</aside>
...
> **Traditional（传统做法）**：Review capacity was planned around human output. A PR waits for a reviewer to read all of it, review quality varies with the reviewer's load, and the author chases while the backlog grows.
>
> **AI-native（AI 原生做法）**：All PRs get an identical set of review passes, with findings ranked by severity. Human attention moves up a level, to whether the change does what the plan intended and whether the risk is acceptable.**Getting started · 起步条件**

- **Prerequisites（前置）**：An updated `CLAUDE.md` file from Stage 3: Build; skills if the review passes enforce written policies, defined subagents.
- **Infrastructure（基础设施）**：A repo with the Claude integration installed, either the managed [Code Review](https://code.claude.com/docs/en/code-review) (research preview) service enabled by an admin or the [claude-code-action](https://code.claude.com/docs/en/github-actions) running in your own CI, with model calls through AWS Bedrock, Google Vertex or Microsoft Foundry where needed (the CI/CD play covers the deployment options). Branch protection policies that require a code owner's approval are also worthwhile.
...
- **Leading indicator（先行指标）**：Time to first review, which should fall to minutes, and the share of review comments resolved without a human touching the branch with data stored directly on Git.
- **Lagging indicator（滞后指标）**：Defects and vulnerabilities caught before merge set against those escaping to production, from the PR history and the incident tracker.
...
- **Prerequisites（前置）**：None.
- **Infrastructure（基础设施）**：A written list of the approvals the change process requires.
...
> **Traditional（传统做法）**：Pipelines run deterministic scripts, and anything that needs judgment waits for a human. For example, triaging the flaky test, writing the changelog, or working out why the build broke. Deployment and rollback are runbooks a human follows under pressure.
>
> **AI-native（AI 原生做法）**：Claude runs non-interactively inside the pipeline for the judgment steps, in a sandbox with scoped credentials. Deployment tooling is exposed to the agent through MCP, so the workflow that wrote and tested the change can also ship it and roll it back, inside gates the organization defines per environment.**Getting started · 起步条件**

- **Prerequisites（前置）**：Claude in the PR review loop and hooks as approval gates, because the gates must exist before automation accelerates anything through them.
- **Infrastructure（基础设施）**：A CI platform with the claude-code-action installed, or any runner that can call `claude -p`; model access through the API, or Bedrock, Foundry, or Vertex where traffic must stay on the organization's cloud agreement; MCP servers for the deployment targets; a sandbox profile for agent jobs with no standing production credentials.
...
- **Leading indicator（先行指标）**：The share of pipeline failures triaged without paging a human taken from the CI/CD pipeline logs.
- **Lagging indicator（滞后指标）**：DevOps Research and Assessment (DORA) measures, which the CI system and deployment tooling already emit.

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · Stage 6 · Maintain——闭环：无人触发的循环</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">循环收口。① Closing the loop：确定性脚本盯生产指标，控制带击穿时按 σ 分级响应——1σ 只记录、2σ 只读诊断、3σ 允许行动（但只能开 PR 进审查门或触发预批准 runbook）；诊断产出写成 intent.md 回到 Stage 1，循环自我供给。检测保持纯确定性（无模型参与），分级配置版本受控。② 定期代码库扫描：Claude Security 托管扫描按计划跑，每个 finding 先验证再报告并附置信度；单 PR 能修的走审查门，架构级弱点写成 intent.md 从 Plan 开始。③ Claude Tag 值班：Claude 以独立身份进驻 Slack 事故频道做第一响应人，诊断、请求授权、执行演练过的 rollback、写 post-mortem 入版本库——频道即审计记录。</p>
</aside>
...
- **Prerequisites（前置）**：`Intent.md` which gives the loop a structured output to restart. Claude accelerated PR reviews, hooks as an action boundary, and a rollback path for CI/CD (which the highest autonomy tier invokes).
- **Infrastructure（基础设施）**：A metrics store the detection script can query (Prometheus, the CI system's API, or equivalents), read access to the repository, a way to run Claude Code non-interactively in CI, or the [Agent SDK](https://platform.claude.com/docs/en/agent-sdk/overview) for a service that receives webhooks.
...
- **Leading indicator（先行指标）**：Time from band breach to an `intent.md` in the triage queue, against the old time from incident to post-mortem action. The detection script's log has the breach timestamp and tier of incident.
- **Lagging indicator（滞后指标）**：The share of findings that become merged fixes (triage queue against actual PR history), and repeat incidents of the same class, which should fall as the fixes add cases to the eval suite.
...
> **Traditional（传统做法）**：Security scanning is an event with a scan launched before a release or an audit. The report goes to a tracker, and the backlog is worked down by hand until the next event. Code written in between is covered by whatever the PR review caught.
>
> **AI-native（AI 原生做法）**：Scans run on a schedule against every connected repository, on the most capable model available, with findings validated before anyone reads them. Each finding is handled the way a breached control band is: a fix that fits in one PR goes through the review gate, and anything larger becomes an `intent.md`. Coverage is dated from the last run, not from the first**Getting started · 起步条件**

- **Prerequisites（前置）**：The PR review gate and hooks as approval gates (Stage 5: Deploy), so that findings go through review like any other change. The `intent.md` format from Stage 1: Plan for findings too large for a single PR.
- **Infrastructure（基础设施）**：Claude Security is available to Claude Enterprise organizations in public beta. It needs the Anthropic GitHub App installed on the target repositories (cloud-hosted github.com), Claude Code on the Web enabled, Extra Usage turned on with a spend limit set, premium seats for the people who run scans, and the feature switched on by an admin at `claude.ai/admin-settings/claude-code`. Scans are billed on consumption at Mythos 5 rates, so the spend limit should match the size and number of repositories.
...
- **Leading indicator（先行指标）**：Share of connected repositories on a schedule, and time from a finding being reported to its patch entering the PR review gate, read from the scan history and the PR metadata.
- **Lagging indicator（滞后指标）**：Vulnerabilities found by the scheduled scan set against those found in production or by external report, from the incident tracker; and the trend in findings per scan on repositories that have been through several runs, which should fall as fixes and evals accumulate.
...
![Claude Tag 在事故频道中作为独立身份成员响应线上事故](/learn/ai-native-sdlc-playbook/claude-tag-oncall.png)

*图：Claude Tag 以独立身份进驻 #inc-checkout 事故频道——被 @ 后 3 分钟内定位根因（新缓存 key 丢了 tenant id），请求授权后执行已在 staging 演练过的回滚。频道本身就是审计记录。*

The channel is the audit trail: request, diagnosis, human authorization and fix all stay where the incident was handled.

<aside class="not-prose my-8 px-6 py-5 bg-accent-purple/5 border-l-4 border-accent-purple rounded-r">
<h3 class="text-sm font-bold text-accent-purple tracking-wide uppercase mb-2">📌 章节导读 · 结语与资源清单</h3>
<p class="text-[15px] text-accent-gray-800 leading-relaxed">转型的关键不是替换人的判断，而是让人的判断留在循环之上：发起、指挥、治理。文末附平台团队落地所需的完整文档清单（admin setup → settings → 权限 → sandbox → hooks → skills → 插件市场 → managed MCP → 企业部署 → 监控 → 合规 API → 安全模型），按建议的 rollout 顺序排列，可直接作为企业级 Claude Code 管控的实施 checklist。</p>
</aside>
...
No items found.PrevPrev0/5NextNexteBook
