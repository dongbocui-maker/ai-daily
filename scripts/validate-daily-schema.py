...
def validate(filepath):
    ...
    try:
        ...
        return False, [f'[JSON_PARSE_ERROR] JSON 解析失败: {e}']

    # 1. 顶层字段
    required_top = ['date', 'title', 'sections', 'closing']
    for f in required_top:
        if f not in d:
            problems.append(f'[MISSING_TOP_FIELD] 缺顶层字段: {f}')

    # 2. title 非空
    if not d.get('title') or not d['title'].strip():
        problems.append(f'[EMPTY_TITLE] title 字段为空: {d.get("title")!r}')

    # 3. sections 数量
    sections = d.get('sections', [])
    if not isinstance(sections, list):
        problems.append(f'[SECTIONS_NOT_ARRAY] sections 不是数组')
    elif len(sections) > 4:
        problems.append(f'[SECTIONS_TOO_MANY] sections 数量 {len(sections)} > 4（疑似重复污染）')
    elif len(sections) < 3:
        problems.append(f'[SECTIONS_TOO_FEW] sections 数量 {len(sections)} < 3（板块不全）')

    # 4. 每个 section 内部
    for i, s in enumerate(sections):
        if 'key' not in s:
            problems.append(f'[MISSING_SECTION] sections[{i}] 缺 key')
        if 'items' not in s or not isinstance(s['items'], list):
            problems.append(f'[MISSING_SECTION] sections[{i}] 缺 items 或不是数组')
            continue
        if len(s['items']) < 3:
            problems.append(f'[SECTION_ITEMS_TOO_FEW] sections[{i}].{s.get("key","?")} items 仅 {len(s["items"])} 条 < 3')
        # 5. 每个 item
        for j, it in enumerate(s['items']):
            for k in ['title', 'body', 'insight', 'source', 'url']:
                if k not in it or not it[k]:
                    problems.append(f'[ITEM_MISSING_FIELD] sections[{i}].{s.get("key","?")} items[{j}] 缺: {k}')
            if 'body' in it and isinstance(it['body'], str) and len(it['body']) < 100:
                problems.append(f'[BODY_TOO_SHORT] sections[{i}].{s.get("key","?")} items[{j}] body 仅 {len(it["body"])} 字 < 100')

    # 6. closing 段数与字数
    closing = d.get('closing', [])
    if not isinstance(closing, list):
        problems.append(f'[CLOSING_NOT_ARRAY] closing 不是数组')
    elif len(closing) > 10:
        problems.append(f'[CLOSING_TOO_MANY] closing 段数 {len(closing)} > 10（疑似单字符序列化污染——参考 5/18 事故）')
    elif len(closing) < 3:
        problems.append(f'[CLOSING_TOO_FEW] closing 段数 {len(closing)} < 3（总结不全）')
    else:
        for i, c in enumerate(closing):
            if not isinstance(c, str):
                problems.append(f'[CLOSING_NOT_STRING] closing[{i}] 不是字符串')
            elif len(c) < 50:
                problems.append(f'[CLOSING_SEGMENT_TOO_SHORT] closing[{i}] 仅 {len(c)} 字 < 50（疑似异常拆分）')
